...
export default class Birch extends Tree {
  ...
  buildLog() {
    ...
    // frustum
    new Log({ position: this.getBlockPosition(0,0,0), r: 0, textureSideIndex: getTextureIndex(TEXTURES.BIRCH_LOG), textureTopIndex: getTextureIndex(TEXTURES.BIRCH_LOG_TOP), b: 0})
    new Log({ position: this.getBlockPosition(0,1,0), r: 0, textureSideIndex: getTextureIndex(TEXTURES.BIRCH_LOG), textureTopIndex: getTextureIndex(TEXTURES.BIRCH_LOG_TOP), b: 0})
    new Log({ position: this.getBlockPosition(0,2,0), r: 0, textureSideIndex: getTextureIndex(TEXTURES.BIRCH_LOG), textureTopIndex: getTextureIndex(TEXTURES.BIRCH_LOG_TOP), b: 0})
    ...
    for (let z = 0; z < 4; z++) {
      ...
      for (let i = -b; i <= b; i++) {
        for (let j = -b; j <= b; j++) { 
          ...
          new Block({ position: p, textureIndex: getTextureIndex(TEXTURES.BIRCH_LEAVES),r: p.z === 0 ? 1 : 0 })
        }
      }
    }
